...
class DiscreteAudioIO(AbsIO):
    ...
    def __init__(
        ...
        device: str = "cpu",
        _skip_loading: bool = False,  # Internal flag for worker copies
    ):
        ...
        Args:
            ...
            _skip_loading: Internal flag to skip model loading for worker copies.
        ...
        if _skip_loading:
            # Worker copy - models and metadata will be copied from parent
            self.codec_model = None
            self.ssl_model = None
            self.km_model = None
            # Metadata will be copied by copy_for_worker
        else:
            # Normal initialization - load models
        ...
        if codec_choice is None:
            ...
            if codec_hf_model_tag is not None:
                ...
                model = AudioCoding.from_pretrained(
                    codec_hf_model_tag, device=str(self.device), token=os.environ["HF_TOKEN"]
                ).model
            ...
            self.codec_model = XcodecModel.from_pretrained(codec_hf_model_tag, token=os.environ["HF_TOKEN"])
            self.codec_n_streams = min(
    ...
    def _codec_decode_batch(
        ...
        if self.codec_choice == "ESPnet":
            ...
            # force approve decoding to be in range [0, 1023]
            print("codes shape: {}; range: {}".format(codes.shape, (codes.min(), codes.max())))
            print("number of negative codes: {}".format((codes < 0).sum()))
            # turn -1 to 0
            codes = torch.where(codes < 0, torch.zeros_like(codes), codes)
    ...
    def _codec_encode_batch(
        ...
        if self.codec_choice == "ESPnet":
            ...
            max_len = int(length.max().item())
            data_trimmed = data[:, :1, :max_len]  # force to use 1 channel for xcodec
    ...
    def preprocess(
        ...
        if sr != self.sample_rate:
            ...
            # print(f"Warning: Sample rate is not compatible for discrete audio, will resample to {self.sample_rate}")
            wav = librosa.resample(wav, orig_sr=sr, target_sr=self.sample_rate)
        if len(wav.shape) > 1:
            wav = wav[0:1, :]  # force to use 1 channel for discrete audio
    ...
    def copy_for_worker(self) -> "DiscreteAudioIO":
        ...
        Creates a lightweight copy that reuses metadata from the parent
        instance instead of loading models from HuggingFace again.
        This avoids overwhelming HuggingFace servers when multiple
        workers are spawned.

        Returns:
            Lightweight copy suitable for workers (no HF queries)
        """
        ...
        worker_copy = self.__class__(
            ...
            _skip_loading=True,  # Skip model loading
        )
        ...
        # Copy metadata from parent instance (already loaded)
        # Codec metadata
        worker_copy.codec_n_streams = self.codec_n_streams
        worker_copy.codec_vocab_size = self.codec_vocab_size
        worker_copy.codec_sample_rate = self.codec_sample_rate
        worker_copy.codec_frame_shift = self.codec_frame_shift
        worker_copy.codec_frame_per_second = self.codec_frame_per_second
        if hasattr(self, 'codec_bandwidth'):
            worker_copy.codec_bandwidth = self.codec_bandwidth

        # SSL metadata
        worker_copy.ssl_n_streams = self.ssl_n_streams
        worker_copy.ssl_vocab_size = self.ssl_vocab_size
        worker_copy.ssl_sample_rate = self.ssl_sample_rate
        worker_copy.ssl_frame_shift = self.ssl_frame_shift
        worker_copy.ssl_frame_per_second = self.ssl_frame_per_second

        # Combined metadata
        worker_copy.sample_rate = self.sample_rate
        worker_copy.frame_shift = self.frame_shift
        worker_copy.frame_per_second = self.frame_per_second
        worker_copy.stream_weights = self.stream_weights
        worker_copy._stream_intervals = self._stream_intervals
        worker_copy.vocabulary = self.vocabulary
        worker_copy.audio_pad = self.audio_pad
...
class ContinuousAudioIO(AbsIO):
    ...
    def __init__(
        ...
        encoder_local_path: str = None,
        attn_implementation: str = None,
        dtype: str = "bfloat16",
        device: str = "cpu",
        _skip_loading: bool = False,
    ):
        """Initialize continuous audio encoder.

        Args:
            encoder_choice: Type of encoder ("huggingface" or "AFWhisper")
            encoder_hf_model_tag: HuggingFace model identifier
                (e.g., "Qwen/Qwen2.5-Omni-7B")
            encoder_local_path: Local path to model weights and config
                (required for "AFWhisper" encoder choice)
        ...
        self.encoder_local_path = encoder_local_path
        ...
        # self._init_encoder()
        if _skip_loading:
            # Worker copy - model, processor, and metadata will be copied from parent
            self.model = None
            self.processor = None
            # Metadata will be copied by copy_for_worker
        else:
            # Normal initialization - load encoder
            self._init_encoder()
    ...
    def _init_encoder(self):
        ...
        if self.encoder_choice == "huggingface":
            if self.encoder_hf_model_tag == "Qwen/Qwen2.5-Omni-7B":
                ...
                self.processor = Qwen2_5OmniProcessor.from_pretrained(
                    self.encoder_hf_model_tag, token=os.environ["HF_TOKEN"]
                ).feature_extractor
        ...
        elif self.encoder_choice == "AFWhisper":
            if self.encoder_local_path is None:
                raise ValueError(
                    "encoder_local_path must be provided for AFWhisper encoder choice"
                )

            from models.ualm.multimodal_io.modeling_whisper import AFWhisperEncoder
            from transformers import WhisperFeatureExtractor

            # Load AFWhisper encoder from local path
            self.model = AFWhisperEncoder.from_pretrained(
                self.encoder_local_path,
                torch_dtype=self.dtype,
            ).to(self.device)
            self.model.eval()

            # Load feature extractor - use Whisper's feature extractor
            # Try to load from local path first, fall back to default Whisper config
            try:
                self.processor = WhisperFeatureExtractor.from_pretrained(
                    self.encoder_local_path
                )
            except Exception:
                # Fall back to default Whisper feature extractor settings
                print(f"Failed to load Whisper feature extractor from {self.encoder_local_path}, using default settings")
                self.processor = WhisperFeatureExtractor(
                    feature_size=self.model.config.num_mel_bins,
                    sampling_rate=16000,
                    hop_length=160,
                    n_fft=400,
                    padding_value=0.0,
            ...
            self.sample_rate = self.processor.sampling_rate
            self.hop_length = self.processor.hop_length
            # Calculate max samples based on max_source_positions
            # max_source_positions is the max sequence length after conv layers
            # The conv layers downsample by factor of 2, so max input frames = max_source_positions * 2
            self.n_samples = self.model.config.max_source_positions * 2 * self.hop_length
    ...
    def preprocess(
        ...
        if fs != self.sample_rate:
            ...
            # print(f"Warning: Imcompatible sampling rate {fs} != {self.sample_rate}")
            wav = librosa.resample(wav, orig_sr=fs, target_sr=self.sample_rate)

        # if wav.shape[0] != 1:
            # raise ValueError("Only support single-channel audio")
            # print(f"Warning: Only support single-channel audio, will use first channel")
        if len(wav.shape) > 1:
            wav = wav[0]

        if wav.shape[0] > self.n_samples:
            print(f"Warning: Input audio is too long to process, will truncate to {self.n_samples} samples")
            wav = wav[:self.n_samples]

        # Ensure wav is a numpy array (not a torch tensor or other type)
        if not isinstance(wav, np.ndarray):
            wav = np.array(wav)

        if self.encoder_choice == "AFWhisper":
            # Store original length for calculating valid output length
            original_samples = wav.shape[0]

            # AFWhisper expects fixed-length input (n_samples = 30 seconds)
            # Pad shorter audio to this length for efficient fixed-size batch processing
            if original_samples < self.n_samples:
                wav = np.pad(wav, (0, self.n_samples - original_samples), mode='constant', constant_values=0)

            # WhisperFeatureExtractor computes mel spectrogram and pads to fixed length
            output = self.processor(
                wav,  # WhisperFeatureExtractor expects raw array, not list
                sampling_rate=self.sample_rate,
                return_tensors="np",
            )

            # Keep FULL mel features (fixed 3000 frames for 30 seconds)
            # This ensures consistent tensor shapes for efficient batching
            # Shape: [1, num_mel_bins, 3000] -> [3000, num_mel_bins]
            feat = output["input_features"][0].T

            # Calculate valid mel frame length based on original audio (before padding)
            # This is used for attention masking in encode_batch to ignore padded regions
            before_length = original_samples // self.hop_length

        else:
            # Original Qwen2.5-Omni path
            output = self.processor(
                [wav],
                truncation=False,
                return_tensors="np",
        ...
        # This matches _get_feat_extract_output_lengths in sound_encoder.py
        after_length = (before_length - 1) // 2 + 1  # First downsample (conv layer)
        after_length = (after_length - 2) // 2 + 1  # Second downsample (conv layer)

        paddings = np.zeros((after_length, 1)).astype(np.int32)

        # Always return after_length (output features) in tuple for consistent placement
        # encode_batch will recompute before_length for attention masking if needed
    ...
    def encode_batch(
        ...
        if self.encoder_choice == "AFWhisper":
            # AFWhisper encoder expects input of shape [batch, num_mel_bins, seq_len]
            # Input is fixed-size (3000 mel frames for 30 seconds)
            # Output is variable-size based on actual audio length:
            #   - 30s audio (3000 mel frames) → 750 output features
            #   - 10s audio (1000 mel frames, padded to 3000) → 250 output features
            batch_size, _, max_mel_seq_len = batch_data.shape

            # length contains OUTPUT feature counts (after_length), need to recompute
            # input mel frame counts (before_length) for attention masking
            # Inverse of: after = (((before - 1) // 2 + 1) - 2) // 2 + 1
            # Approximate inverse: before ≈ 4 * after - 1
            input_mel_lengths = length * 4 - 1

            # Compute feature lengths after conv layers from input mel lengths
            # audio_feat_lengths: after first conv layer (e.g., 500 for 10s)
            # audio_output_lengths: after second conv layer (e.g., 250 for 10s)
            audio_feat_lengths, audio_output_lengths = self.model._get_feat_extract_output_lengths(
                input_mel_lengths
            )

            # Create 4D attention mask for the encoder
            # max_seq_len is based on PADDED input (always 3000 mel frames → 1500 after first conv)
            # Mask ensures encoder ignores padded regions beyond audio_feat_lengths
            max_seq_len = (max_mel_seq_len - 2) // 2 + 1
            seq_range = (
                torch.arange(0, max_seq_len, dtype=audio_feat_lengths.dtype, device=batch_data.device)
                .unsqueeze(0)
            ...
            # Forward pass through AFWhisper encoder with attention mask
            encoder_output = self.model(batch_data, attention_mask=audio_attention_mask)
            audio_features = encoder_output.last_hidden_state  # [batch, max_output_len, d_model]

            # Extract only valid features (no padding) based on actual audio length
            # e.g., for 10s audio: extract first 250 features, discard remaining 500
            audio_features_list = []
            for i in range(batch_size):
                audio_features_list.append(audio_features[i, :audio_output_lengths[i], :])
    ...
    def find_length(self, data: Tuple[np.ndarray, int]) -> int:
        """Calculate output frame length after encoding.
        ...
        Returns:
            Output frame length after encoding (after_length).
            This is the number of output features after encoder downsampling.
        """
        wav, sr = data
        # Handle sample rate conversion
        if sr != self.sample_rate:
            num_samples = int(wav.shape[-1] * self.sample_rate / sr)
        else:
            num_samples = wav.shape[-1]

        frame_length = num_samples // self.hop_length  # Initial mel frames

        # Apply same downsampling as the encoder model (both AFWhisper and Qwen)
    ...
    def copy_for_worker(self) -> "ContinuousAudioIO":
        ...
        Creates a lightweight copy that reuses the processor and metadata
        from the parent instance instead of loading models from HuggingFace
        again. This avoids overwhelming HuggingFace servers when multiple
        workers are spawned.
        ...
        worker_copy = self.__class__(
            ...
            encoder_local_path=self.encoder_local_path,
            attn_implementation=self.attn_implementation,
        ...
        # Copy the processor (needed for preprocessing) and metadata from parent
        worker_copy.processor = self.processor
        worker_copy.d_model = self.d_model
        worker_copy.sample_rate = self.sample_rate
        worker_copy.hop_length = self.hop_length
        worker_copy.n_samples = self.n_samples
...
def test_afwhisper_continuous_audio_io(af_whisper_path: str = "/path/to/af-whisper"):
    """Test AFWhisper ContinuousAudioIO with different audio lengths.
    
    Tests that:
    - 30s audio → 750 output features
    - 10s audio → 250 output features (no padding in output)
    - 5s audio → 125 output features
    
    Args:
        af_whisper_path: Local path to AF-Whisper model
    """
    print("=" * 60)
    print("Testing AFWhisper ContinuousAudioIO")
    print("=" * 60)
    
    # Initialize the encoder
    print(f"\n1. Loading AFWhisper from: {af_whisper_path}")
    audio_io = ContinuousAudioIO(
        encoder_choice="AFWhisper",
        encoder_local_path=af_whisper_path,
        dtype="float32",
        device="cuda" if torch.cuda.is_available() else "cpu",
    )
    print(f"   - Sample rate: {audio_io.sample_rate}")
    print(f"   - Hop length: {audio_io.hop_length}")
    print(f"   - Max samples (30s): {audio_io.n_samples}")
    print(f"   - Feature dim: {audio_io.d_model}")
    
    # Test different audio durations
    test_durations = [5, 10, 20, 30]  # seconds
    
    print(f"\n2. Testing preprocessing with different durations:")
    for duration in test_durations:
        # Create synthetic audio
        num_samples = duration * audio_io.sample_rate
        wav = np.random.randn(num_samples).astype(np.float32)
        
        # Preprocess
        paddings, (after_length, feat), loss_mask = audio_io.preprocess((wav, audio_io.sample_rate))
        
        # Expected values
        expected_mel_frames = num_samples // audio_io.hop_length
        expected_output_len = (expected_mel_frames - 1) // 2 + 1  # after conv1
        expected_output_len = (expected_output_len - 2) // 2 + 1  # after conv2
        
        print(f"   {duration}s audio:")
        print(f"      - Input samples: {num_samples}")
        print(f"      - Mel features shape: {feat.shape} (fixed 3000 frames for 30s)")
        print(f"      - Output features (after_length): {after_length}")
        print(f"      - Expected output features: {expected_output_len}")
        assert after_length == expected_output_len, f"Mismatch! Got {after_length}, expected {expected_output_len}"
    
    # Test batch encoding
    print(f"\n3. Testing batch encoding:")
    device = audio_io.device
    
    # Create batch with different durations
    batch_durations = [10, 20, 30]  # seconds
    batch_feats = []
    batch_lengths = []  # These are now after_length (output feature counts)
    
    for duration in batch_durations:
        num_samples = duration * audio_io.sample_rate
        wav = np.random.randn(num_samples).astype(np.float32)
        _, (after_length, feat), _ = audio_io.preprocess((wav, audio_io.sample_rate))
        batch_feats.append(feat)
        batch_lengths.append(after_length)
    
    # Stack into batch tensor
    batch_data = torch.tensor(np.stack(batch_feats, axis=0), dtype=audio_io.dtype, device=device)
    batch_lengths = torch.tensor(batch_lengths, dtype=torch.long, device=device)
    
    print(f"   - Batch data shape: {batch_data.shape}")
    print(f"   - Batch lengths (output features): {batch_lengths.tolist()}")
    
    # Encode batch
    audio_io.model.eval()
    with torch.no_grad():
        audio_features = audio_io.encode_batch(batch_data, batch_lengths)
    
    print(f"\n   Output features (no padding included):")
    for i, (duration, features) in enumerate(zip(batch_durations, audio_features)):
        expected_output = batch_lengths[i].item()
        print(f"      - {duration}s audio: shape={features.shape}, expected_len={expected_output}")
        assert features.shape[0] == expected_output, f"Mismatch! Got {features.shape[0]}, expected {expected_output}"
